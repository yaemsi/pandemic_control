...
class SIR_Env(Base_Env):
    def __init__(self, cfg: Dict | os.PathLike) -> None:
        ...
        if not hasattr(self, f"S0"):
            self.S0 = self.N - sum([getattr(self, f"{k}0", 0) for k in 'IR'])
        for comp in 'SIR':
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        ...
        #  Update observation
        y = self.S, self.I, self.R #on sauvegarde old observation        
        ret = odeint(self.deriv, y, self.time) 

        self.S, self.I, self.R = ret[-1]
        observation = np.array([self.S, self.I, self.R], dtype=np.float32)
        
        if round(self.S + self.I + self.R) != self.N:
    ...
    def reward(self, action: int) -> float:
        ...
        if (self.N*0.25<self.I): 
            eco_cost = eco_cost/2 # Reduce the importance of the economy
        else:
            #health_cost = health_cost/2 # Reduce the importance of the health
            health_cost = 0
    ...
    def reset(self, seed: int | None = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        self.steps = 0
        self.S, self.I, self.R =  self.S0, self.I0, self.R0
        self.beta = 0.8
        
        # Reinitializing the history
        self.list_S = [] 
        self.list_I = [] 
        self.list_R = [] 
        self.economic_cost =[]
    ...
    def update_history(self, ret:np.ndarray, rew: float, action: float) -> None:
        ...
        self.list_I = [*self.list_I, *ret[1:].T[1]]
        self.list_R = [*self.list_R, *ret[1:].T[2]]
        self.list_actions += self.days*[int(action)]
        self.list_betas += self.days*[self.beta]
        self.rewards += self.days*[rew]

    
    def deriv(self, y: Tuple[float], t: int) -> Tuple[float, float, float]:
        S, I, R = y
