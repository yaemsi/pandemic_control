...
class SEIRADH_Env(Base_Env):
    ...
    def __init__(self, cfg: dict | os.PathLike) -> None:
        ...
        if not hasattr(self, "S0"):
            self.S0 = self.N - sum(
                [getattr(self, f"{k}0", 0) for k in ["E", "I_a", "I_s", "R", "D", "H"]]
            )
    ...
    def step(
        ...
        y = (
            ...
            self.R,
            self.D,
        )  # on sauvegarde old observation
        ret = odeint(self.deriv, y, self.time)

        self.S, self.E, self.I_a, self.I_s, self.H, self.R, self.D = ret[-1]

        observation = np.array(
            [self.S, self.E, self.I_a, self.I_s, self.H, self.R, self.D],
            dtype=np.float32,
        )

        if (
            round(self.S + self.E + self.I_a + self.I_s + self.H + self.R + self.D)
            != self.N
        ):
    ...
    def deriv(
        self, y: tuple[float], t: int
    ) -> tuple[float, float, float, float, float, float]:
        S, E, I_a, I_s, H, R, D = y
        p_s, p_h, p_d, p_dh = self.probs
        N, beta, gamma, delta, theta, mu, sigma = (
            self.N,
            self.beta,
            self.gamma,
            self.delta,
