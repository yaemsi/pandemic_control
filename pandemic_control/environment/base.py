...
class Base_Env(gym.Env):
    def __init__(self, cfg: dict | os.PathLike) -> None:
        super().__init__()
        if not cfg:
            raise ValueError("Configuration file/path cannot be nil")
        elif isinstance(cfg, dict):
            self.config = deepcopy(cfg)
        elif isinstance(cfg, str):
            if not cfg.endswith(".json"):
                raise ValueError(
                    "Only json formats are supported for configuration files."
                )
        ...
        self.observation_space = spaces.Box(
            low=0, high=self.N, shape=(n_compartments,), dtype=np.float32
        )
