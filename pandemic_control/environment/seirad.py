...
class SEIRAD_Env(Base_Env):
    def __init__(self, cfg: dict | os.PathLike) -> None:
        ...
        if not hasattr(self, "S0"):
            self.S0 = self.N - sum(
                [getattr(self, f"{k}0", 0) for k in ["E", "I_a", "I_s", "R", "D"]]
            )
    ...
    def step(
        ...
        observation = np.array(
            [self.S, self.E, self.I_s, self.I_a, self.R, self.D], dtype=np.float32
        )
    ...
    def reset(self, seed: int | None = None) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        self.steps = 0
        self.beta = 0.8
        self.S, self.E, self.I_s, self.I_a, self.R, self.D = (
            self.S0,
            self.E0,
            self.I_s0,
            self.I_a0,
