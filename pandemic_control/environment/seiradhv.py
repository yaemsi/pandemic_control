...
class SEIRADHV_Env(Base_Env):
    def __init__(self, cfg: dict | os.PathLike) -> None:
        ...
        #  SEIRADHV parameters
        if not hasattr(self, "I_a0"):
            self.I_a0 = 0
        if not hasattr(self, "I_s0"):
            self.I_s0 = 1
        ...
        if not hasattr(self, "S0"):
            self.S0 = self.N - sum(
                [
                    getattr(self, f"{k}0", 0)
                    for k in ["E", "I_a", "I_s", "R", "D", "H", "V"]
                ]
            )
        ...
        if (
            (not hasattr(self, "gamma"))
            or (not isinstance(self.gamma, list))
            or (len(self.gamma) != 3)
        ):
            self.gamma = [1.0 / 10, 1.0 / 15]
    ...
    def update_initial_conditions(
        ...
        if V0 < 0 or E0 < 0 or I_a0 < 0 or I_s0 < 0 or H0 < 0 or R0 < 0 or D0 < 0:
            raise ValueError("Error: the initial variables should all be positive")
        elif V0 + E0 + I_a0 + I_s0 + H0 + R0 + D0 > self.N:
            raise ValueError(
                f"Error: the sum of initial variables '{s}' should be equal to the population '{self.N}'"
            )
        else:
            self.V0 = V0
            self.E0 = E0
    ...
    def step(
        ...
        observation = np.array(
            [self.S, self.V, self.E, self.I_a, self.I_s, self.H, self.R, self.D],
            dtype=np.float32,
        )
        if np.round(observation.sum()) != self.N:
            raise Exception(
                f"The sum of compartments ({round(observation.sum()) }) isn't equal to N ({self.N})"
            )
