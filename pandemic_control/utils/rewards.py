...
def reward(self, action, prev_obs):
    ...
    return (
        self.trade_off_weights[0] * health_cost + self.trade_off_weights[1] * eco_cost
    )
...
def reward_arango_pelov(self, action, prev_obs):
    self.health_cost += self.days * [
        self.trade_off_weights[0] * health_reward_arango_pelov(self, prev_obs)
    ]
    self.economic_cost += self.days * [
        self.trade_off_weights[1] * economy_reward_arango_pelov(self, action, prev_obs)
    ]
    return self.health_cost[-1] + self.economic_cost[-1]
...
class RewardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.locals["dones"][0]:
            ...
            if self.verbose > 0:
                logger.info(
                    f"Episode finished, total reward: {self.episode_rewards[-1]}"
                )
        return True
