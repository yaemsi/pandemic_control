...
def economy_reward(env: Base_Env, action: float, prev_obs: list[float]):
...
def economy_reward_dynamic(env: Base_Env, action: float, prev_obs: list[float]):
    # Percentage of active population multiplied by the action.
    economic_contrib = (env.N - (env.I_a + env.I_s + env.D + env.H)) * (
        env.action[int(action)][1]
    )

    return economic_contrib / env.N


def economy_reward_arango_pelov(env: Base_Env, action: float, prev_obs: list[float]):
    if action != 0:
        return -0.1
    else:
        return 0
...
def health_reward(env: Base_Env, prev_obs: list[float]):
    ...
    health_cost = env.infection_cost[-1] + env.hospt_cost[-1] + env.death_cost[-1]
