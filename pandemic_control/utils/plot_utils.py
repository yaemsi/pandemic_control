...
def plot_env_dynamics(
    ...
    p = sns.lineplot(
        data=plot_data[['Days'] + labels].set_index('Days'), 
        ax=axis, 
        dashes=None, 
        linewidth=2, 
        linestyle='-', 
...
def plot_curves(
    ...
    for lbl in labels:
        ...
        sns.lineplot(
            y=f"{lbl}", 
            x=f"{x_axis}", 
            data=plot_data[[f'{x_axis}'] + labels].set_index(f'{x_axis}'), 
...
"""
Rewards collected during training, as recorded by 'RewardCallback'. One column
per algorithm, one row per step: the index is used as the x axis, so pass
'cb.step_rewards' for the per-step signal or 'cb.episode_rewards' for one point
per episode (adjust 'xlabel' accordingly).

Columns are coloured after 'models_cmap' when they are named after a known
algorithm. Curves do not need to share the same length, shorter ones simply
being padded with NaNs.
"""
def plot_rewards(
    plot_data: pd.DataFrame,
    rootdir: str | os.PathLike,
    filename: str = 'rewards',
    labels: List[str] = [],
    figsize: Tuple[int, int] = (10, 8),
    facecolor: str = "#ffffff",
    xlabel: str = 'Steps',
    ...
    missing = [l for l in labels if not l in plot_data.columns]
    if missing:
        raise ValueError(f"Invalid dataframe. Column(s) {missing} not found.")

    if plot_data[labels].dropna(how='all').empty:
        raise ValueError(
            f"No rewards to plot. Check that training actually ran and that the "
            f"callback was passed to 'model.train()'."
            )
    ...
    for i, lbl in enumerate(labels):
        ...
        color = models_cmap.get(f"{lbl}", f"C{i}")
    ...
    axis.legend(title=legend_title, loc=f"{legend_loc}", fontsize=14)
