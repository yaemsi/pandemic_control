...
def plot_curves(
    ...
    if "Environment" not in plot_data.columns:
        raise ValueError("Invalid dataframe. 'Environment' column not found.")
    ...
    if not labels:
        ...
        if rem:
            raise ValueError(
                f"Following column keys were not found in the dataframe '{rem}'. Please \
    ...
    for lbl in labels:
        ...
        if separated:
            _, axis = plt.subplots(
                ncols=1, nrows=1, figsize=figsize, facecolor=facecolor
            )
        ...
        if annotate_max:
            # Annotating peaks
            for attribute, place in zip(
                global_annotation_map[env_name][0], global_annotation_map[env_name][1]
            ):
                if place:
                    annot_max(
                        plot_data.Days,
                        getattr(plot_data, f"{attribute}"),
                        place=place,
                        ax=axis,
                    )
                else:
        ...
        if lbl == "Hospitalized":
            if hosp_cap:
                if "Hosp_Cap" not in plot_data.columns:
                    raise ValueError("Invalid dataframe. 'Hosp_Cap' column not found.")
                max_cap = plot_data["Hosp_Cap"][0]
                axis.axhline(
                    max_cap, color="r", linestyle="--", label=r"$C_{h}$", linewidth=2.5
                )
    ...
    if not separated:
        ...
        if prefixname:
            ...
            filepath = os.path.join(rootdir, "all.pdf")
        plt.savefig(filepath, pad_inches=0, bbox_inches="tight", transparent=True)
        if show_plot:
            plt.show()
        plt.close()
...
def action_piechart(
    ...
    perc_actions = np.divide(actions_per_cat, np.sum(actions_per_cat))
    ...
    axis.legend(
        wedges, labels, title="Selected actions", bbox_to_anchor=(0.85, 1), loc="best"
    )
...
def plot_with_variance(
    plot_data: pd.DataFrame,
    labels: list[str] = ["Infected", "Deceased", "Hospitalized"],
...
def actions_span(
    ...
    if matrix[pst][1] not in handels_label:
        handles.append(
            mpatches.Patch(color=matrix[pst][0], label=matrix[pst][1], alpha=0.6)
        )
        handels_label.append(matrix[pst][1])
    leg = Legend(
        axis, handles, handels_label, loc=best_loc, fancybox=True, framealpha=0.5
    )
    axis.add_artist(leg)
...
def plot_categorized_dynamics(
    ...
    plot_curves(
        ...
        labels=[
            x
            for x in ["Susceptible", "Vaccinated"]
            if x in env_labels_map[f"{env_name}"]
        ],
        show_plot=show_plot,
    )
    ...
    plot_curves(
        ...
        labels=[
            x
            for x in ["Hospitalized", "Deceased", "Recovered"]
            if x in env_labels_map[f"{env_name}"]
        ],
        show_plot=show_plot,
    )
    ...
    plot_curves(
        ...
        labels=[
            x
            for x in ["Infected", "Exposed", "Symptomatic", "Asymptomatic"]
            if x in env_labels_map[f"{env_name}"]
        ],
        show_plot=show_plot,
    )
...
def plot_env_metrics(
    ...
    if env_name == "SEIRADHV":
        ...
        plot_cum_critic_metrics(
            plot_data, rootdir, f"{filename}_cum_critics", show_plot
        )
    ...
    # Actions
    plot_actions(plot_data, rootdir, "actions", show_plot)

    # Actions in a piechart
    action_piechart(plot_data, rootdir, "piechart", show_plot)
...
def plot_cum_critic_metrics(
    ...
    plot_curves(
        plot_data=plot_data,
        rootdir=rootdir,
        prefixname=f"{filename}_rec_vac_cum" if filename else "rec_vac_cum",
...
def compare_metrics(
    ...
    for label in label_map.keys():
        ...
        sns.lineplot(
            ...
            y=label,
            data=predicted,
            color=dynamics_cmap[label],
            ax=axis,
            label=label,
            linewidth=2.5,
        ).set(xlabel="Days", ylabel=None)
        # Actual numbers
        sns.lineplot(
            x="Days",
            y=label_map[label],
            data=actual,
            color=dynamics_cmap[f"{label}_ref"],
            ax=axis,
        ...
        if label == "Hospitalized":
            if "Hosp_Cap" not in predicted.columns:
                raise ValueError("Invalid dataframe. 'Hosp_Cap' column not found.")
        ...
        filepath = os.path.join(
            f"{rootdir}",
            f"{filename}_{label}.pdf" if filename else f"comparison_{label}.pdf",
        )
