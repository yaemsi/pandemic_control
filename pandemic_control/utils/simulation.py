...
def run_env_sim(
    ...
    if mode == "Test":
        ...
        if not model:
            raise ValueError(
                f"'Predict' mode requires a model as input. Found '{model}' instead."
            )
        for _ in range(t_max):
        ...
        raise ValueError(
            f"Unknwon mode '{mode}'. Modes supported are : 'Test', 'Random' and 'Predict'"
        )
...
def run_simulation(env: Base_Env, model: BaseModel, t_max: int) -> dict[str, Any]:
...
def run_n_simulations(
    env: Base_Env, model: BaseModel, rounds: int, t_max: int
) -> dict[str, Any]:
